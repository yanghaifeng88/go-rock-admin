...
func RegUserRouter(r *gin.Engine) {
	...
	r.Handle("POST", "/user/create", func() func(ctx *gin.Context) {
	...
	r.Handle("POST", "/user/login", func() func(ctx *gin.Context) {
		return func(ctx *gin.Context) {
	...
	r.Handle("GET", "/user/info", func(ctx *gin.Context) {
		result, code, err := ctrl.Info(ctx)
		egin.Response(ctx, result, code, err)
	})

	r.Handle("POST", "/user/logout", func(ctx *gin.Context) {
		result, code, err := ctrl.Logout(ctx)
		egin.Response(ctx, result, code, err)
	})

	r.Handle("GET", "/user/routes", func(ctx *gin.Context) {
		result, code, err := ctrl.Routes(ctx)
		egin.Response(ctx, result, code, err)
	})

	r.Handle("GET", "/user/list", func() func(ctx *gin.Context) {
		return func(ctx *gin.Context) {
	...
	r.Handle("GET", "/user/get/:id", func(ctx *gin.Context) {
		id, _ := strconv.Atoi(ctx.Param("id"))
		result, code, err := ctrl.Get(ctx, id)
		egin.Response(ctx, result, code, err)
	}, middleware.IpLimiter())

	r.Handle("POST", "/user/update/:id", func() func(ctx *gin.Context) {
		return func(ctx *gin.Context) {
			var params controller.UserForm
			errs := utils.Validated(ctx, &params)
			if errs != nil {
	...
	r.Handle("DELETE", "/user/delete/:id", func(ctx *gin.Context) {
		id, _ := strconv.Atoi(ctx.Param("id"))
